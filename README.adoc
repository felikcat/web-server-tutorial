...
`sudo ufw default deny incoming; ufw enable`
...
== Tips
If you end up enforcing all of apparmor.d's rules, here's what I recommend putting on no enforcement and no warnings: +
`sudo aa-disable sudo apt apt-methods-file apt-extracttemplates apt-listchanges dpkg-split dpkg-deb unattended-upgrade aa-notify`

* Disabling the `sudo` apparmor.d profile is my preference due to it requiring you to enter your password on every single `sudo` command ran, which gets annoying fast.
